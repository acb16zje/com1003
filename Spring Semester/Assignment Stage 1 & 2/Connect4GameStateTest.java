...
public class Connect4GameStateTest {
	...
	private void checkMove(String msg, Connect4GameState gs, int col, int row, int counter) {
		...
		try {
			gs.move(col);
			assertEquals(message + "move made incorrectly - checking ("+col+","+row+")", counter, gs.getCounterAt(col, row));
			assertEquals(message + "next turn counter not changed correctly", counter == RED ? YELLOW : RED, gs.whoseTurn());
		} catch (ColumnFullException e) {
			fail(message + "ColumnFullException raised when column not full");
		} catch (IllegalColumnException e) {
			fail(message + "IllegalColumnException raised on legal column");
		}
	}
}
